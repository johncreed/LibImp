...
struct Option
{
    ...
    ImpInt verbose;
    string data_path, test_path, test_path_2, treat_path;
...
string train_help()
{
    ...
    "-treat <path>: set path to treat set\n"
...
int main(int argc, char *argv[])
{
    try
    {
        ...
        shared_ptr<ImpData> treat_data = make_shared<ImpData>(option.treat_path);
        ...
        if (!treat_data->file_name.empty()) {
            treat_data->read();
        }

        ImpProblem prob(data, treat_data, test_data, option.param);
        prob.initialize();
        prob.solve();
    }
