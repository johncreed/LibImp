...
void ImpProblem::initialize() {
    ImpLong m = max(treat_data->m, data->m), n = max(treat_data->n ,data->n);
    m_tc = max(treat_data->m, data->m), n_tc = max(treat_data->n ,data->n);
    ImpInt k = param->k;
    t = 0;
    ...
    H = impMalloc(n*k);
    HT = impMalloc(n*k);
    W_treat = impMalloc(m*k);
    WT_treat = impMalloc(m*k);
    H_treat = impMalloc(n*k);
    HT_treat = impMalloc(n*k);

    gamma_w.resize(n,0);
    gamma_h.resize(m,0);
    gamma_w_treat.resize(n,0);
    gamma_h_treat.resize(m,0);

    default_random_engine engine(0);
    uniform_real_distribution<ImpFloat> distribution(0.0, 1.0/sqrt(k));
    for (ImpInt d = 0; d < k; d++)
    {
        for (ImpLong j = 0; j < m; j++) {
            WT[d*m+j] = distribution(engine); 
            W[j*k+d] = WT[d*m+j];
            WT_treat[d*m+j] = distribution(engine); 
            W_treat[j*k+d] = WT_treat[d*m+j];
        }
        for (ImpLong j = 0; j < n; j++) {
            HT[d*n+j] = distribution(engine);
            H[j*k+d] = HT[d*n+j];
            HT_treat[d*n+j] = distribution(engine);
            H_treat[j*k+d] = HT_treat[d*n+j];
        }
        for (ImpLong j = 0; j < data->n; j++) {
            if (data->RT.row_ptr[j+1]!=data->RT.row_ptr[j]) {
                HT[d*n+j] = 0.0;
                H[j*k+d] = HT[d*n+j];
            } 
        }
        for (ImpLong j = 0; j < treat_data->n; j++) {
            if (treat_data->RT.row_ptr[j+1]!=treat_data->RT.row_ptr[j]) {
                HT_treat[d*n+j] = 0.0;
                H_treat[j*k+d] = HT_treat[d*n+j];
            } 
        }
...
void ImpProblem::update(const smat &R, ImpLong i, vector<ImpFloat> &gamma, ImpFloat *u, ImpFloat *v, const ImpFloat lambda, const ImpDouble w_p, const ImpDouble u_i_R) {
    ImpFloat a = param->a;
    //ImpFloat w = param->w;
    //ImpInt k = param->k;
    ImpDouble u_val = u[i];
    ImpDouble lambda2 = param->lambda_treat;
    assert(R.row_ptr.size() > (ImpuLong) i+1);
    ImpDouble h = (lambda)*(R.row_ptr[i+1] - R.row_ptr[i]) + lambda2*1.0, g = lambda2*u_i_R;
...
void ImpProblem::update_R(shared_ptr<ImpData> cur_data, ImpDouble *wt, ImpDouble *ht, bool add) {
    smat &R = cur_data->R;
    smat &RT = cur_data->RT;
    ImpLong m = cur_data->m, n = cur_data->n;
    if (add) {
#pragma omp parallel for schedule(guided)
        for (ImpLong i = 0; i < m; i++) {
            ImpDouble w = wt[i];
...
void ImpProblem::update_coordinates() {
    ...
    for (ImpInt d = 0; d < k; d++) {
         ImpDouble *u = &WT[d*m];
         ImpDouble *v = &HT[d*n];
         ImpDouble *u_treat = &WT_treat[d*m];
         ImpDouble *v_treat = &HT_treat[d*n];
         ImpDouble *ut = &W[d];
         ImpDouble *vt = &H[d];
         time = omp_get_wtime();
         update_R(data, u, v, true);
...
#pragma omp parallel for schedule(dynamic)
            for (ImpLong j = 0; j < data->n; j++) {
                if (data->RT.row_ptr[j+1]!=data->RT.row_ptr[j])
                    update(data->RT, j, gamma_w, v, u, param->lambda_c, param->w, v_treat[j]);
            }
...
//            cout << "Solve v" << endl;
#pragma omp parallel for schedule(dynamic)
            for (ImpLong i = 0; i < data->m; i++) {
                if (data->R.row_ptr[i+1]!=data->R.row_ptr[i])
                    update(data->R, i, gamma_h, u, v, param->lambda_c, param->w, u_treat[i]);
            }
            update_time += omp_get_wtime() - time;
...
#pragma omp parallel for schedule(static)
        ...
        time = omp_get_wtime();
        update_R(data, u, v, false);
        r_time += omp_get_wtime() - time;
    }
}

void ImpProblem::update_coordinates_treat() {
    ImpInt k = param->k;
    ImpLong m = m_tc, n = n_tc;
    double cache_time = 0.0;
    double update_time = 0.0;
    ...
    double time, time2;
    for (ImpInt d = 0; d < k; d++) {
         ImpDouble *u = &WT_treat[d*m];
         ImpDouble *v = &HT_treat[d*n];
         ImpDouble *u_ctrl = &WT[d*m];
         ImpDouble *v_ctrl = &HT[d*n];
         ImpDouble *ut = &W_treat[d];
         ImpDouble *vt = &H_treat[d];
         time = omp_get_wtime();
         update_R(treat_data, u, v, true);
         r_time += omp_get_wtime() - time;
         time2 = omp_get_wtime();
         for (ImpInt s = 0; s < 5; s++) {
            time = omp_get_wtime();
            cache(WT_treat, H_treat, gamma_w_treat, u, m, n);
            cache_time += omp_get_wtime() - time;
            time = omp_get_wtime();
//            cout << "Solve u (T)" << endl;
#pragma omp parallel for schedule(dynamic)
            for (ImpLong j = 0; j < treat_data->n; j++) {
                assert(treat_data->RT.row_ptr.size() > (ImpuLong)j+1);
                if (treat_data->RT.row_ptr[j+1]!=treat_data->RT.row_ptr[j])
                    update(treat_data->RT, j, gamma_w_treat, v, u, param->lambda_t, param->w, v_ctrl[j]);
            }
            update_time += omp_get_wtime() - time;
            //cblas_dcopy(n, v, 1, vt, k);
            time = omp_get_wtime();
#pragma omp parallel for schedule(static)
            ...
            sync_time += omp_get_wtime() - time;
            time = omp_get_wtime();
            cache(HT_treat, W_treat, gamma_h_treat, v, n, m);
            cache_time += omp_get_wtime() - time;
            time = omp_get_wtime();
//            cout << "Solve v (T)" << endl;
#pragma omp parallel for schedule(dynamic)
            for (ImpLong i = 0; i < treat_data->m; i++) {
                assert(treat_data->R.row_ptr.size() > (ImpuLong) i+1);
                if (treat_data->R.row_ptr[i+1]!=treat_data->R.row_ptr[i])
                    update(treat_data->R, i, gamma_h_treat, u, v, param->lambda_t, param->w, u_ctrl[i]);
            }
            update_time += omp_get_wtime() - time;
            //cblas_dcopy(m, u, 1, ut, k);
            time = omp_get_wtime();
...
void ImpProblem::solve() {
    ...
    for (t = 0; t < param->nr_pass; t++) {
        update_coordinates();
        update_coordinates_treat();
        validate_ndcg(topks);
        print_epoch_info();
        //cout << setprecision(3) << sqrt(cal_tr_loss()/data->l) << fixed;
        cout << "MSE: " << setprecision(3) << sqrt(cal_te_loss()/test_data->l) << fixed << endl;
        //cout.width(13);
    }
